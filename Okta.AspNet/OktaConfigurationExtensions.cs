...
namespace Okta.AspNet
{
    ...
    public static class OktaConfigurationExtensions
    {
        ...
        private static void BindWebOptions(IConfigurationSection section, Abstractions.OktaWebOptions options)
        {
            // First, check for Issuer - if set, it will auto-populate OktaDomain and AuthorizationServerId
            var issuer = section[nameof(Abstractions.OktaWebOptions.Issuer)];
            ...
            var oktaDomain = section[nameof(Abstractions.OktaWebOptions.OktaDomain)];
            ...
            var authorizationServerId = section[nameof(Abstractions.OktaWebOptions.AuthorizationServerId)];
            if (!string.IsNullOrEmpty(authorizationServerId))
            {
                options.AuthorizationServerId = authorizationServerId;
            }

            var clockSkewValue = section[nameof(Abstractions.OktaWebOptions.ClockSkew)];
            ...
            var backchannelTimeoutValue = section[nameof(Abstractions.OktaWebOptions.BackchannelTimeout)];
        ...
        {
            var audience = section[nameof(Abstractions.OktaWebApiOptions.Audience)];
        ...
        private static void BindMvcOptions(IConfigurationSection section, OktaMvcOptions options)
        {
            ...
            var redirectUri = section[nameof(OktaMvcOptions.RedirectUri)];
            ...
            var useTokenLifetime = section[nameof(OktaMvcOptions.UseTokenLifetime)];
            if (!string.IsNullOrEmpty(useTokenLifetime) && bool.TryParse(useTokenLifetime, out var useTokenLifetimeValue))
            {
                options.UseTokenLifetime = useTokenLifetimeValue;
            }

            var loginMode = section[nameof(OktaMvcOptions.LoginMode)];
            if (!string.IsNullOrEmpty(loginMode) && Enum.TryParse<LoginMode>(loginMode, ignoreCase: true, out var loginModeValue))
            {
                options.LoginMode = loginModeValue;
            }
